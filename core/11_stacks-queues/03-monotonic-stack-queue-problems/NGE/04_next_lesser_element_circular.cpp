/*
Given a circular integer array arr, return the next smaller element for every element in arr.
...
// TC: O(n); SC: O(n)
vector<int> circular_NLE(vector<int> &arr)
{
    stack<int> st;
    int n = arr.size();
    vector<int> res(n);
...
int main()
{
    ...
    vector<int> result = circular_NLE(arr);
    cout << endl
